...
/***
 * <p>
 * Test for {@link MemcacheRestCache}. Uses jmemcached, which doesn't currently
 * honor TTLs.
 * </p>
 */
public class MemcacheRestCacheTest {
    ...
    private static HashMap<String, Collection<String>> newHeaders() {
        ...
        map.put("Cache-Control", Lists
                .newArrayList("no-transform,public,max-age=2,s-maxage=900"));
    ...
    public void testSetAndGet() throws InterruptedException {
        ...
        EVCache evCache = (new EVCache.Builder()).setAppName("SAMPLECACHE")
                .setCacheName("cid").enableZoneFallback().build();

        final MemcacheRestCache memcacheRestCache = new MemcacheRestCache(
                evCache);
        final Map<String, Collection<String>> headers = newHeaders();
        memcacheRestCache.set(FOO, new CachedResponse(HttpStatus.SC_OK,
                headers, FOO.getBytes()));

        final CachedResponse cachedResponse = memcacheRestCache.get(FOO).get();

        Assert.assertThat(cachedResponse.getStatus(),
                IsEqual.equalTo(HttpStatus.SC_OK));
        Assert.assertThat(cachedResponse.getCachedBytes(),
                IsEqual.equalTo(FOO.getBytes()));
    ...
    public void testSetException() throws EVCacheException {
        ...
        CachedResponse cachedResponse = new CachedResponse(HttpStatus.SC_OK,
                null, null);
        ...
        try {
            ...
            Assert.assertThat(
                    e.getClass(),
                    IsEqual.equalTo(MemcacheRestCache.MemcacheRestCacheException.class));
            Assert.assertThat(e.getMessage(),
                    IsEqual.equalTo("Could set key foo"));
        }
    }
    ...
    public void testGetException() throws EVCacheException {
        ...
        Mockito.doThrow(new EVCacheException("Get me an exception!"))
                .when(evcache).get(FOO, MemcacheRestCache.TRANSCODER);
        ...
        try {
            ...
            Assert.assertThat(
                    ...
                    IsEqual.equalTo(MemcacheRestCache.MemcacheRestCacheException.class));
            Assert.assertThat(e.getMessage(),
                    IsEqual.equalTo("Could get key foo"));
        }
    }

    /**
     * verify that when TTL is under 2592000 seconds, the value is NOT converted
     * to a unix timestamp.
     *
    ...
    public void testSetTTLSeconds() throws EVCacheException {
        ...
        final CachedResponse cachedResponse = Mockito
                .mock(CachedResponse.class);
        final MemcacheRestCache memcacheRestCache = new MemcacheRestCache(
                evCache);
        final ArgumentCaptor<Integer> ttlCaptor = ArgumentCaptor
                .forClass(Integer.class);

        Mockito.when(cachedResponse.getTTL()).thenReturn(2591999L);
        memcacheRestCache.set(key, cachedResponse);

        Mockito.verify(evCache).set(Matchers.eq(key), Matchers.any(),
                Matchers.any(ObjectTranscoder.class), ttlCaptor.capture());
    ...
    private class ObjectTranscoder extends SerializingTranscoder implements
            EVCacheTranscoder<Object> {

    }

    /**
     * Verify that when TTL is over 2592000L, a unix timestamp is used as the
     * memcache TTL.
     *
     * @throws EVCacheException
     */
